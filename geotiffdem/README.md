...
This project contains source code and configuration for a serverless
application that you can deploy with the __AWS Serverless Application Model__
...
The application uses several AWS resources, including Lambda functions, an API
Gateway API, and Amazon DynamoDB tables. These resources are defined in the
`template.yml` file in this project. You can update the template to add AWS
resources through the same deployment process that updates your application
code.
...
The AWS Toolkit is an open-source plugin for popular IDEs that uses the AWS SAM
CLI to build and deploy serverless applications on AWS. The AWS Toolkit also
adds step-through debugging for Lambda function code.
...
- __Stack Name__: The name of the stack to deploy to CloudFormation. This should
  be unique to your account and region, and a good starting point would be
- something matching your project name.
- __AWS Region__: The AWS region you want to deploy your app to.
- __Confirm changes before deploy__: If set to yes, any change sets will be
  shown to you before execution for manual review. If set to no, the AWS SAM CLI will automatically deploy application changes.
- __Allow SAM CLI IAM role creation__: Many AWS SAM templates, including this
  ...
  permissions. To deploy an AWS CloudFormation stack which creates or modified
...
- __Save arguments to samconfig.toml__: If set to yes, your choices will be
...
The AWS SAM CLI reads the application template to determine the API's routes and
...
The application template uses AWS SAM to define application resources. AWS SAM
...
For resources that aren't included in the
...
__Resources__ section, add a resource named __MyQueue__ with the type
__AWS::SQS::Queue__. Then add a property to the __AWS::Serverless::Function__
resource named __DeadLetterQueue__ that targets the queue's Amazon Resource Name
(ARN), and a policy that grants the function permission to access the queue.
...
processed. It's only used if you invoke your function asynchronously, but it's
useful here to show how you can modify your application's resources and function
configuration.
...
Open the
[__Applications__](https://console.aws.amazon.com/lambda/home#/applications)
page of the Lambda console, and choose your application. When the deployment
completes, view the application resources on the __Overview__ tab to see the new
resource. Then, choose the function to see the updated configuration that
specifies the dead-letter queue.

## Fetch, tail, and filter Lambda function logs

To simplify troubleshooting, the AWS SAM CLI has a command called `sam logs`.
`sam logs` lets you fetch logs that are generated by your Lambda function from
the command line. In addition to printing the logs on the terminal, this command
has several nifty features to help you quickly find the bug.

__NOTE:__ This command works for all Lambda functions, not just the ones you
...
__NOTE:__ This uses the logical name of the function within the stack. This is
the correct name to use when searching logs inside an AWS Lambda function
within a CloudFormation stack, even if the deployed function name varies due to
...
```bash
aws cloudformation delete-stack --stack-name geotiffdem
```
